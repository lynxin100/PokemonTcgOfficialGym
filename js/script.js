function initClock(elt, opt_properties) {
	const defaultProperties = {color: '#0ff', date: new Date(), glow: true, militaryTime: true, showMs: true};
	...
	const getSecondsPassed = (now = c.date, militaryTime = c.militaryTime) => {
		...
		return {  hour: h, minute10: m10, hour: h, minute10: m10, minute1: Math.floor((now.getMinutes() % 10) * 60 + s10), second10: s10, second1: Math.floor(now.getSeconds() % 10) };
	};
...
initClock(document.querySelector("#clock"), {color: '#006cb8', dotColor: '#006cb8', showMs: false});
